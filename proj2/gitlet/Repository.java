...
public class Repository {
    ...
    public static TreeMap<String, byte[]> blobs = new TreeMap<>();  // blobs.
    public static TreeMap<String, String> removedSets = new TreeMap<>();
    ...
    public static void init() {
        ...
        GITHEADS_DIR.mkdir();  // Create heads directory.
        GIT_BRANCH_DIR.mkdir();  // Create branches/ directory.

        File head = join(GITHEADS_DIR, "master"); // Read the sha-1 string value of branch head into refs/head
    ...
    public static void add(String filename) {
        ...
        Commit head = readObject(join(GITHEADS_DIR, plainFilenamesIn(GITHEADS_DIR).get(0)), Commit.class);
...
//        newCommit.addPreviousCommitTrack(head.getTrack());
        ...
        writeObject(join(GITHEADS_DIR, currentBranchName), head);
    ...
    public static void rm(String filename) {
        ...
        // TODO: bug exists. If current directory no such a file, error will occur.
        ...
        if (stageArea.containsValue(filename)) {
            ...
            // If the file is neither tracked by the head commit nor stage in stage area. Print error message.
    ...
    public static void status() {
        ...
        Commit headCommit = readObject(join(GITHEADS_DIR, plainFilenamesIn(GITHEADS_DIR).get(0)), Commit.class);
    ...
    public static void checkout(String args[]) {
        String currentBranchName = plainFilenamesIn(GITHEADS_DIR).get(0);
        Commit currCommit = readObject(join(GITHEADS_DIR, currentBranchName), Commit.class);
        ...
        if (args.length == 3) {
            ...
            // Traverse one file tracked by this commit to find the blobId corresponding to the filename.
            for (Map.Entry<String, String> entry : tracks.entrySet()) {
                if (entry.getValue().equals(fileName)) {
        ...
        // checkout commitId -- fileName
            ...
            List<String> commitIdNames = plainFilenamesIn(GITOBJECTS_DIR);
            // If current commit tracks do not contain the commit id, print message.
            if (!commitIdNames.contains(commitId)) {
            ...
            Commit concreteCommit = readObject(join(GITOBJECTS_DIR, commitId), Commit.class);
        ...
        // checkout branchName
            ...
            writeObject(join(GITHEADS_DIR, branchName), branchToBeChanged);

            // Delete current branch file from heads/ dir.
            join(GITHEADS_DIR, currentBranchName).delete();
    ...
    public static void reset(String commitId) {
        List<String> commitIds = plainFilenamesIn(GITOBJECTS_DIR);
        String currentBranchName = plainFilenamesIn(GIT_BRANCH_DIR).get(0);
        stageArea = readObject(GITSTAGEAREA, TreeMap.class);
        blobs = readObject(GIT_BLOB, TreeMap.class);
        ...
        for (String fileName : currentDirFileNames) {
            ...
            // TODO
            if (!tracks.containsValue(fileName)) {
                // delete files of current directory that does not be tracked by the to be reset commit.
                restrictedDelete(concreteFile);
            }
        }

        // Traverse files tracked by this commit to find the blobId corresponding to the filename and write file.
        for (Map.Entry<String, String> entry : tracks.entrySet()) {
            String fileName = entry.getValue();
            byte[] fileContentTrackedByCommit = blobs.get(entry.getKey());
            writeContents(join(CWD, fileName), fileContentTrackedByCommit);
        ...
        // Rollback branches, then update the branch serialized file in heads/ dir and branches/ dir.
        writeObject(join(GITHEADS_DIR, currentBranchName), concreteCommit);
